...
- **Day-node anchoring.** Tana creates two nodes per capture: the bullet you see in your day node and a hidden audio node. Every note is anchored on the bullet (`tana_id`); the audio node is remembered as `tana_memo_id` and never written to. `--relink` migrates older archives.
